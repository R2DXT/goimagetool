...
Usage:
  ...
  goimagetool load kernel-fit <itbPath> [compression]     # applies to the outer file
...
func main() {
	...
	for i < len(args) {
		...
		case "load":
			...
			p := args[i+2]
			comp := "auto"
			if (typ == "initramfs" || typ == "kernel-fit" || typ == "ext2") && i+3 < len(args) {
				comp = args[i+3]
				i++
			}
			switch typ {
		...
		case "store":
			...
			case "ext2":
				...
				bs := 1024
				comp := "none"
				// optional [blockSize] [compression]
				if i+3 < len(args) {
					nxt := args[i+3]
					if isDigits(nxt) {
						fmt.Sscanf(nxt, "%d", &bs)
