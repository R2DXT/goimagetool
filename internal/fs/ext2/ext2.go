...
// Extended EXT2 support:
// - Read: 1K/2K/4K block sizes; full directory traversal; direct + 1/2/3-level
//   indirects; 128-byte inodes (classic ext2)
// - Write: single block group; 1K/2K/4K; multi-block files/dirs; builds
//   direct + single/double/triple-indirect pointer trees.
...
func Load(r io.Reader) (*memfs.FS, error) {
	...
	var sb superblock
	if err := readAt(b, 1024, &sb); err != nil {
		return nil, err
	}
	if sb.Magic != ext2Magic {
		return nil, errors.New("not ext2")
	}
	if sb.RevLevel < 1 {
		return nil, errUnsupported
	}
	bsz, err := getBlockSize(&sb)
	if err != nil {
		return nil, err
	}
	...
	if err := readAt(b, gdtBlock*bsz, &gd); err != nil {
		return nil, err
	}
	...
	getInode := func(n uint32) (inode, error) {
		...
		if err := readAt(b, off, &ino); err != nil {
			return inode{}, err
		}
		return ino, nil
	}

	// Expand block pointers recursively (direct + 1/2/3 indirect)
	...
	expandBlocks = func(p uint32, level int) ([]uint32, error) {
		...
		if err := binary.Read(bytes.NewReader(b[off:off+bsz]), binary.LittleEndian, &ptrs); err != nil {
			return nil, err
		}
	...
	walkDir = func(inum uint32, parent string) error {
		ino, err := getInode(inum)
		if err != nil {
			return err
		}
		if ino.Mode&modeIFDIR == 0 {
			return fmt.Errorf("inode %d not a dir", inum)
		}
		data, err := readFileData(&ino)
		if err != nil {
			return err
		}
		p := 0
		for p+8 <= len(data) {
			var de dirent
			if err := binary.Read(bytes.NewReader(data[p:p+8]), binary.LittleEndian, &de); err != nil {
				return err
			}
			if de.Inode == 0 || de.RecLen == 0 {
				break
			}
			...
			nlen := int(de.NameLen)
			if 8+nlen > int(de.RecLen) || p+8+nlen > len(data) {
				break
			}
			...
			child, err := getInode(de.Inode)
			if err != nil {
				return err
			}
			full := path.Join(parent, name)
			mt := time.Unix(int64(child.MTime), 0)
			if de.FileType == 2 || (child.Mode&modeIFDIR) != 0 {
				fs.PutDirMode(full, memfs.Mode(child.Mode), uint32(child.UID), uint32(child.GID), mt)
				if err := walkDir(de.Inode, full); err != nil {
					return err
				}
			} else if de.FileType == 1 || (child.Mode&modeIFREG) != 0 {
				data, err := readFileData(&child)
				if err != nil {
					return err
				}
				fs.PutFile(full, data, memfs.Mode(child.Mode), uint32(child.UID), uint32(child.GID), mt)
			}
		}
		return nil
	}
...
func Store(w io.Writer, fs *memfs.FS, blockSize int) error {
	...
	case 2048, 4096:
		bsz = blockSize
	default:
		return fmt.Errorf("unsupported blockSize: %d (use 1024/2048/4096)", blockSize)
	}
	...
	// Assign inode numbers: 2=root, then dirs, then files
	inodeNum := uint32(3)
	for _, r := range all {
	 if r.IsDir {
	  r.Inode = inodeNum
	...
	// Build directory payload (packed entries; padded to block size)
	buildDirPayload := func(selfIno uint32, dirPath string) []byte {
		...
		for idx, e := range list {
			...
			de := dirent{
				Inode:    e.ino,
				RecLen:   uint16(useRec),
				NameLen:  uint8(nl),
				FileType: e.ft,
			}
			var hdr bytes.Buffer
			_ = binary.Write(&hdr, binary.LittleEndian, de)
			out.Write(hdr.Bytes())
			out.WriteString(e.nm)
			used := 8 + nl
			for used < useRec {
				out.WriteByte(0)
				used++
	...
	// Prepare directory payloads
	dirPayload := map[uint32][]byte{}
	for _, r := range all {
		if r.IsDir {
			dirPayload[r.Inode] = buildDirPayload(r.Inode, r.Path)
		}
	}
	...
	// choose metadata placement depending on block size
	gdtBlk := 2
	if bsz > 1024 {
		gdtBlk = 1
	}
	...
	planForDataBlocks := func(n int) plan {
		...
		if rem > 0 {
			use := rem
			if use > ptrsPer { use = ptrsPer }
			if use > 0 { p.ind1 = -1; rem -= use }
		}
		if rem > 0 {
			cap2 := ptrsPer * ptrsPer
			use := rem
			if use > cap2 { use = cap2 }
			if use > 0 {
				p.ind2Root = -1
				p.ind2L1 = make([]int, ceilDiv(use, ptrsPer))
				rem -= use
			}
		}
		if rem > 0 {
			...
			use := rem
			if use > cap3 { use = cap3 }
			if use > 0 {
				p.ind3Root = -1
				p.ind3L2 = make([]int, ceilDiv(use, ptrsPer*ptrsPer))
				p.ind3L1 = make([]int, ceilDiv(use, ptrsPer))
				rem -= use
			}
		}
		return p
	...
	// Layout image total blocks (we'll derive from allocations)
	...
	for _, r := range all {
		pl := filePlan[r.Inode]
		if r.IsDir {
			pl.data = allocData(blocksFor(len(dirPayload[r.Inode])))
		} else {
			pl.data = allocData(blocksFor(len(r.Data)))
		}
		allocPtr(pl)
	}

	totalBlocks := curBlk // first free block index == count
	...
	switch bsz {
	case 1024: sb.LogBlockSize = 0
	case 2048: sb.LogBlockSize = 1
	case 4096: sb.LogBlockSize = 2
	}
	...
	// helper to mark pointer blocks
	markPtr := func(pl *plan) {
		if pl.ind1 > 0 { markBlk(pl.ind1) }
		if pl.ind2Root > 0 {
			markBlk(pl.ind2Root)
			for _, x := range pl.ind2L1 { markBlk(x) }
		}
		if pl.ind3Root > 0 {
			markBlk(pl.ind3Root)
			for _, x := range pl.ind3L2 { markBlk(x) }
			for _, x := range pl.ind3L1 { markBlk(x) }
		}
	}
	markPtr(&rootPlan)
	for _, r := range all {
	...
	// Build pointer blocks for an inode + set inode.Block[]
	writePointers := func(pl *plan, dataBlocks []int, ino *inode) {
		...
		if pl.ind1 > 0 {
			...
			n := len(dataBlocks) - idx
			if n > ptrsPer { n = ptrsPer }
			for i := 0; i < n; i++ { ptrs[i] = uint32(dataBlocks[idx+i]) }
			var buf bytes.Buffer
			_ = binary.Write(&buf, binary.LittleEndian, ptrs)
		...
		if pl.ind2Root > 0 {
			...
			_ = binary.Write(&rbuf, binary.LittleEndian, root)
			copy(img[pl.ind2Root*bsz:], rbuf.Bytes())
			for i := 0; i < len(pl.ind2L1); i++ {
				l1 := make([]uint32, ptrsPer)
				for j := 0; j < ptrsPer && idx < len(dataBlocks); j++ {
					l1[j] = uint32(dataBlocks[idx])
					idx++
				}
				var b bytes.Buffer
				_ = binary.Write(&b, binary.LittleEndian, l1)
		...
		if pl.ind3Root > 0 {
			...
			for l2i := 0; l2i < len(pl.ind3L2); l2i++ {
				...
				countL1 := ptrsPer
				if l1Idx+countL1 > len(pl.ind3L1) {
					countL1 = len(pl.ind3L1) - l1Idx
					if countL1 < 0 { countL1 = 0 }
				}
				for k := 0; k < countL1; k++ { l1List[k] = uint32(pl.ind3L1[l1Idx+k]) }
				var l2buf bytes.Buffer
				_ = binary.Write(&l2buf, binary.LittleEndian, l1List)
				copy(img[pl.ind3L2[l2i]*bsz:], l2buf.Bytes())

				for k := 0; k < countL1; k++ {
					l1ptrs := make([]uint32, ptrsPer)
					for j := 0; j < ptrsPer && idx < len(dataBlocks); j++ {
						l1ptrs[j] = uint32(dataBlocks[idx]); idx++
					}
					var lb bytes.Buffer
					_ = binary.Write(&lb, binary.LittleEndian, l1ptrs)
